...
function SubjectContent() {
    ...
    if (error) {
        return (
            <main className="relative min-h-screen w-full flex flex-col justify-center items-center bg-background text-foreground" dir="ltr">
                <div className="relative z-10 w-full max-w-sm rounded-[2.5rem] bg-white/[0.03] backdrop-blur-2xl border border-red-500/20 p-10 text-center shadow-2xl">
                    ...
                    <button onClick={() => router.push(`/student`)} className="w-full h-14 bg-white/10 hover:bg-white text-foreground hover:text-black font-bold rounded-2xl transition-all duration-300">
                        Back
                    </button>
                </div>
            </main>
    ...
    if (isLoading || !data || !subject) {
        return (
            <main className="relative min-h-screen w-full flex flex-col justify-center items-center bg-background" dir="ltr">
    ...
    return (
        <main className={`relative min-h-screen w-full flex flex-col bg-background text-foreground transition-opacity duration-1000 ease-out opacity-100`} dir="ltr">
            ...
            {/* Navigation Header */}
            <nav className="relative z-10 w-full border-b border-border-color bg-background/50 backdrop-blur-2xl print:hidden">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-5 flex justify-between items-center">
                    <div className="flex items-center gap-4 sm:gap-5">
                        <div className="w-10 h-10 sm:w-14 sm:h-14 relative rounded-full overflow-hidden shadow-2xl p-0.5 bg-gradient-to-b from-brand-primary/40 to-transparent border border-border-color">
                        ...
                        <div className="flex flex-col justify-center">
                            <h1 className="text-base sm:text-2xl font-black text-foreground tracking-tight drop-shadow-md truncate max-w-[150px] sm:max-w-none">{data.studentName}</h1>
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <button
                            ...
                            className="px-4 py-2.5 text-xs sm:text-sm font-bold bg-white/5 hover:bg-white/10 border border-border-color rounded-xl transition-all active:scale-95 text-text-muted hover:text-foreground backdrop-blur-sm group flex items-center gap-2"
                        >
            ...
            <div className={`relative z-10 flex-1 w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12 flex flex-col`}>
                ...
                <div className="mb-10 animate-in fade-in slide-in-from-bottom-4 duration-500 print:hidden">
                    <div className="flex items-center gap-4 mb-4">
                        <span className="inline-block px-3 py-1 bg-white/5 border border-border-color rounded-lg text-[10px] font-black text-brand-primary tracking-widest uppercase">
                            {subject.subjectId}
                        </span>
                    </div>
                    <h2 className="text-3xl sm:text-5xl font-black text-foreground tracking-tight leading-tight mb-4 text-transparent bg-clip-text bg-gradient-to-r from-white to-white/60">
                        {subject.subjectName}
                    </h2>
                    <p className="text-sm sm:text-base text-text-muted font-medium max-w-2xl leading-relaxed">
                ...
                {/* Grades Container */}
                <div className="w-full bg-white/[0.02] border border-border-color rounded-[2.5rem] p-5 sm:p-10 shadow-2xl backdrop-blur-sm print:bg-transparent print:border-none print:shadow-none print:p-0">
                    {subject.grades.length === 0 ? (
                        <div className="py-24 flex flex-col items-center justify-center">
                            <FileText className="w-16 h-16 text-foreground/10 mb-5" />
                            <p className="text-text-muted font-bold text-lg text-center max-w-xs">No grade entries available for this module.</p>
                        </div>
                    ) : (
                                    ...
                                    >
                                        <div className="flex-1 mb-4 sm:mb-0">
                                            <div className="flex items-center gap-3 mb-1">
                                                ...
                                                <h4 className="text-xl font-bold text-foreground group-hover:text-brand-primary transition-colors print:text-black">
                                                    {grade.examName}
                                                </h4>
                                            </div>
                                            <p className={`text-[10px] text-text-muted font-bold uppercase tracking-widest pl-5 print:text-gray-400`}>
                                                {dateStr}
                                            </p>
                                        </div>

                                        <div className="flex items-center gap-6 w-full sm:w-auto">
                                            <div className="w-full sm:w-28 py-3 rounded-2xl flex flex-col items-center justify-center border bg-white/5 border-border-color group-hover:border-white/30 transition-all print:bg-transparent print:border-none">
                                                <span className="text-[10px] text-text-muted font-black uppercase tracking-widest mb-1 print:hidden">Score</span>
                                                <span className="text-3xl font-black text-foreground print:text-black">
                                                    {grade.score} <span className="text-sm text-text-muted">/ {grade.maxScore}</span>
                                                </span>
                                            </div>
